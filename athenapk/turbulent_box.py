...
class TurbulentBox:
    ...
    def state_ICs(self):
        ...
        print(f"  Cooling Time: {get_t_cool_n(T0 / (chi_inserted_cloud), rho0 * (chi_inserted_cloud) / self.mbar, self.mbar) * ut.constants.s_to_Myrs * Lambda_units:.3e} Myr")
        print(f"  t_cool,mix / t_eddy = {get_t_cool_n(T0 / (chi_inserted_cloud)**0.5 , rho0 * (chi_inserted_cloud)**0.5  / self.mbar, self.mbar)*Lambda_units / (rcl * self.code_length_cgs * (chi_inserted_cloud)**0.5 / v_turb):.3e}")

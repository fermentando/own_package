...
def compute_vsf_chunk(x, y, z, vx, vz, bin_edges, seed):
    ...
    drs = np.linalg.norm((coords[pairs[:, 0]] - coords[pairs[:, 1]]), axis=1)
    dvs = np.linalg.norm((vels[pairs[:, 0]]*vels[pairs[:, 0]] - vels[pairs[:, 1]]*vels[pairs[:, 1]]), axis=1)

    # ✅ Square the bin edges for correct binning on squared distances
    bin_idx = np.searchsorted(bin_edges, drs) - 1
...
def process_run(infile, stand_l, mbar, outdir, n_jobs):
    ...
    outfile = os.path.join(outdir, f"cold_3D_vsf2_{str(int(idx)).zfill(3)}")
...
if __name__ == "__main__":
    ...
    print(run_paths)
    single_file_paths = sorted(glob.glob(os.path.join(run_paths[0], 'out/parthenon.prim.[0-9]*.phdf')))[::4] #[::-1]

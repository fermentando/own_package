...
def process_blocks_worker(block_indices, filename, LogicalLocations, MeshBlockSize, code_units_rho, code_units_vel, fields):
    results = []
...
def read_hdf5(filename=None, fields=['rho'], n_jobs=1, chunk_size=10, use_dask=False):
    if filename is None:
    ...
    data_coords = {
        'x1': X.transpose(2, 1, 0) * code_units_length,
        'x2': Y.transpose(2, 1, 0) * code_units_length,
        'x3': Z.transpose(2, 1, 0) * code_units_length
    }
    ...
    if use_dask:
        delayed_results = [
            dask.delayed(process_blocks_worker)(
                chunk, filename, LogicalLocations, MeshBlockSize, code_units_rho, code_units_vel, fields
            ) for chunk in block_chunks
        ]

        data = {field: da.zeros((nz, ny, nx), dtype=np.float64) for field in fields}

        def fill_data(res, field):
            arr = np.zeros((nz, ny, nx), dtype=np.float64)
            for (start3, end3, start2, end2, start1, end1, blocks) in res:
                arr[start3:end3, start2:end2, start1:end1] = blocks.get(field, 0)
            return arr

        for field in fields:
            lazy_blocks = [dask.delayed(fill_data)(res, field) for res in delayed_results]
            stacked = da.stack([da.from_delayed(b, shape=(nz, ny, nx), dtype=np.float64)
                                for b in lazy_blocks])
            data[field] = da.nanmax(stacked, axis=0)

        data.update({k: da.from_array(v, chunks='auto') for k, v in data_coords.items()})
        return data

    else:
        with multiprocessing.Pool(processes=n_jobs) as pool:
            results_chunks = pool.starmap(
                process_blocks_worker,
                [(chunk, filename, LogicalLocations, MeshBlockSize, code_units_rho, code_units_vel, fields)
                 for chunk in block_chunks]
            )

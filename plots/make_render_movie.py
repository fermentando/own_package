...
def render_frame_parallel(file_info, hold_frames, rot_frames, spiral_frames, thetas, phi, r_vals, output_dir):
    ...
    frame_path = os.path.join(output_dir, f"black_frame_{globalindx:03d}.png")
...
def rot_and_spiral_zoom_in(files, nhold=0.3, nrot=0, nspiral=0.7, output_dir=None, n_workers=4):
    ...
    # Combine to video
    dir_save = os.path.join(output_dir, "black_volume_rendering.mp4")
    frame_files = sorted(glob.glob(os.path.join(output_dir, "black_frame_*.png")))

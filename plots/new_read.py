...
import utils as ut  # assuming ut is your utility module
import multiprocessing


# Globals to be set before multiprocessing
_file_handle = None
_LogicalLocations = None
_MeshBlockSize = None
_code_units_rho = None
_code_units_vel = None
_fields = None

def init_worker(filename, LogicalLocations, MeshBlockSize, code_units_rho, code_units_vel, fields):
    global _file_handle, _LogicalLocations, _MeshBlockSize, _code_units_rho, _code_units_vel, _fields
    _file_handle = h5py.File(filename, "r")
    _LogicalLocations = LogicalLocations
    _MeshBlockSize = MeshBlockSize
    _code_units_rho = code_units_rho
    _code_units_vel = code_units_vel
    _fields = fields

def process_blocks_worker(block_indices):
    global _file_handle, _LogicalLocations, _MeshBlockSize, _code_units_rho, _code_units_vel, _fields
    prim = _file_handle["prim"]
    results = []

    for i in block_indices:
        lx, ly, lz = _LogicalLocations[i]
        mbl3, mbl2, mbl1 = _MeshBlockSize
        start1 = mbl1 * lx
        start2 = mbl2 * ly
        start3 = mbl3 * lz
        end1 = start1 + mbl1
        ...
        for field in _fields:
            try:
                if field == 'rho':
                    block = prim[i, 0, :, :, :] * _code_units_rho
                elif field == 'prs':
                    block = prim[i, 4, :, :, :] * _code_units_rho * _code_units_vel**2
                elif field == 'vel1':
                    block = prim[i, 1, :, :, :] * _code_units_vel
                elif field == 'vel2':
                    block = prim[i, 2, :, :, :] * _code_units_vel
                elif field == 'vel3':
                    block = prim[i, 3, :, :, :] * _code_units_vel
                elif field == 'T':
                    rho_block = prim[i, 0, :, :, :] * _code_units_rho
                    prs_block = prim[i, 4, :, :, :] * _code_units_rho * _code_units_vel**2
...
def read_hdf5(filename=None, fields=['rho'], n_jobs=1, chunk_size=10):
    if filename is None:
    ...
    with h5py.File(filename, "r") as f:
        ...
        block_chunks = list(chunks(block_indices, chunk_size))

        # Setup globals for workers in initializer
        pool_args = (filename, LogicalLocations, MeshBlockSize, code_units_rho, code_units_vel, fields)

        with multiprocessing.Pool(processes=n_jobs, initializer=init_worker, initargs=pool_args) as pool:
            results_chunks = pool.map(process_blocks_worker, block_chunks)

...
if __name__ == "__main__":
    ...
    run_paths = ['/viper/ptmp/ferhi/LEGACY/fvLism/01kc/fv01_30r', '/viper/ptmp/ferhi/LEGACY/fvLism/01kc/fv01_narrow']
    ...
    for j, run in enumerate(run_paths):
        ...
        tcc = chi**0.5 * 0.1*sim.R_cloud / sim.v_wind
        ...
        if plot_hst:
            ...
            if 'narrow' in run: label = r'$L_\mathrm{\perp, box} = 16 r_\mathrm{cl}$'; color = 'purple'
            if '_30r' in run: label = r'$L_\mathrm{\perp, box} = 32 r_\mathrm{cl}$'; color = 'black'
            ax.plot((timeseries * code_time_cgs - t_shift) / t_linear, norm_mass-norm_mass[correction_index_mass], color=color, linestyle=linestyles[base_fv], alpha=0.8, label=label)
    ...
    ax.set_xlabel(r't [$t_{cc} $]')
    ax.set_ylim(bottom=-2, top=1.)

    # Save and show
    print(f"Saved to: /u/ferhi/Figures/res_test.pdf")
    plt.tight_layout()
    plt.savefig(f'/u/ferhi/Figures/res_test.pdf', dpi = 300, bbox_inches = 'tight')
    plt.show()
